...
public class RtrainInfoController extends ParentTrainInfoController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources){
        ...
        Train rTrain = new Train().getRTrain();
        Label rTrainInfo = new Label();
        rTrainInfo.setText(rTrain.displayDetailedRoot());
        sp.setContent(rTrainInfo);
        storeStops();
    ...
    private void storeStops() {
        Train rTrain = new Train().getRTrain();
        for (int i = 0; i < rTrain.numberOfStops(); i++) possibleWords[i] = rTrain.get(i);
    }
    ...
    public void pressSearch(){
        ...
        int i;
        for ( i=0; i<rTrain.numberOfStops(); i++){
            if (rTrain.get(i).equals(input)){
                station = rTrain.getStation(i);
                break;
            }
        }
