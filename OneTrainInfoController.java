...
public class OneTrainInfoController extends ParentTrainInfoController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources){
        ...
        Train oneTrain = new Train().get1Train();
        Label oneTrainInfo = new Label();
        oneTrainInfo.setText(oneTrain.displayDetailedRoot());
        sp.setContent(oneTrainInfo);
        storeStops();
    ...
    private void storeStops() {
        Train oneTrain = new Train().get1Train();
        for (int i = 0; i < oneTrain.numberOfStops(); i++) possibleWords[i] = oneTrain.get(i);
    }
    ...
    public void pressSearch(){
        ...
        int i;
        for ( i=0; i<oneTrain.numberOfStops(); i++){
            if (oneTrain.get(i).equals(input)){
                station = oneTrain.getStation(i);
                break;
            }
        }
